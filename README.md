...
- 可视化板块：用 KPI、企业排名、技术雷达、应用占比、趋势线和地区热力图展示市场与技术现状。
- 公司板块：按产业链环节展示 Top 10 公司、技术路线、客户关系、相关新闻和关系网图。
...
这些图表用于学习和行业判断训练，数据口径会随公开报告更新继续校准。

## 公司板块说明

公司板块使用 `data/companies.json` 维护产业链地图：

- `segments`：电芯/整包、正极、负极、隔膜、电解液、集流体、粘结剂/导电剂、设备、回收。
- `companies`：每个环节 Top 10 公司，包含排名、份额口径、技术路线、客户和证据等级。
- `relations`：供应、客户、合作、回收闭环等公司关系，用于绘制关系网。

公司份额优先使用公开报告、年报、招股书和行业数据库；缺少一致公开口径时先保留排名和 `shareLabel`，避免伪精确百分比。
